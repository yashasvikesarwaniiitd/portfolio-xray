...
SYSTEM = (
    ...
    "Never ask the user for an AMFI scheme code, and never end a turn with only a question "
    "when a sensible default exists. Resolve fund names yourself: search_fund maps names to "
    "codes, and for funds the user already owns, load_portfolio's symbol column IS the AMFI "
    "code. When search returns variants of the SAME fund that differ only by plan or option "
    "(Direct vs Regular, Growth vs IDCW), do not present a menu — pick Direct Growth, fetch "
    "the NAV, and say in one short line which variant you used and that other variants "
    "exist. Only ask when the matches are genuinely DIFFERENT funds.\n\n"
